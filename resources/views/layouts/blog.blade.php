            ...
            <div class="collapse navbar-collapse" id="edicaMainNav">
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    <li class="nav-item active">
                        <a class="nav-link" href="{{route('blog.index')}}">Блог<span class="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
                ...
                <ul class="navbar-nav mt-2 mt-lg-0">
                    @auth
                        <li class="nav-item">
                            <a href="{{ url('/home') }}" class="nav-link">Личный кабинет</a>
                        </li>
                    @else
                        <li class="nav-item">
...
<script>
    ...
    document.querySelectorAll('.form-liked').forEach( elementForm=> {
        elementForm.onsubmit = (event)=> {
            event.preventDefault();
            let token = elementForm.children.namedItem('_token').value
            fetch(`/blog/${elementForm.dataset.id}/liked`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': token
                },
              ...
              .then(data => {
                  console.log(data)
                  let imgHeart = elementForm.querySelector('i')
                  let spanLike = elementForm.querySelector('span')
                  if(imgHeart.classList.contains('fas')){
                      imgHeart.classList.replace('fas', 'far')
                      spanLike.textContent = String(Number(spanLike.textContent) - 1)
                  } else {
                      imgHeart.classList.replace('far', 'fas')
                      spanLike.textContent = String(Number(spanLike.textContent) + 1)
                  }
              })
        }
